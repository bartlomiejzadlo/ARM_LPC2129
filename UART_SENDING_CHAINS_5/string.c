...
void AppendString(char pcSourceStr[],char pcDestinationStr[]){
    unsigned char ucLengthOfStringCounter;
    for(ucLengthOfStringCounter = 0; pcDestinationStr[ucLengthOfStringCounter] != NULL  ; ucLengthOfStringCounter++){};    
    CopyString(pcSourceStr, pcDestinationStr + ucLengthOfStringCounter);
}
...
void AppendUIntToString(unsigned int uiValue, char pcDestinationStr[]){
	...
	UIntToHexStr(uiValue, &pcDestinationStr[ucSignCounter]);
