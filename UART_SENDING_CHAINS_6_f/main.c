...
int main (){
	...
	unsigned char fCallibTokenDetected;
	unsigned char fGoToTokenDetected;
	...
	while(1){
		...
		if(eReciever_GetStatus() == READY){
			...
			if ((ucTokenNr != 0) && (asToken[0].uValue.eKeyword == CALC)){
				...
				fIDTokenDetected = 1;
			}else if((ucTokenNr != 0) && (asToken[0].uValue.eKeyword == CALLIBRATION)){
				fCallibTokenDetected = 1;
				ServoCallib();
			}else if((ucTokenNr != 0) && (asToken[0].uValue.eKeyword == GOTO)){
				fGoToTokenDetected = 1;
		...
		if (Transmiter_GetStatus() == FREE){
			if(sWatch.fSeccondsValueChanged == 1){		
				...
				Transmiter_SendString(cReceivedTime);
			}else	if (fCallibTokenDetected == 1){
				fCallibTokenDetected = 0;
				CopyString("CALLIBRATION\n ", cReceivedTime);
				Transmiter_SendString(cReceivedTime);
			}else	if (fGoToTokenDetected == 1){ 
				fGoToTokenDetected = 0;
				CopyString("GOTO ", cReceivedTime);
				AppendUIntToString(asToken[1].uValue.uiNumber, cReceivedTime);
				AppendString("\n", cReceivedTime);
				Transmiter_SendString(cReceivedTime);
			}
		}
